...
# define MAX_MESSAGE_SIZE   1000 // maximum number of symbols in a message
#endif
...
struct
{
  ...
  int   * Finalencoded_message;          // the encoded message
...
#define _Finalencoded_message_      decoder_global_data.Finalencoded_message
...
static void recursive_decoder(int encoded_idx,int decoded_idx,int good_decoded_size)
{
  ...
  int verifica=0;
  int i;
  for ( i= 0; i<_original_message_size_;i++ ){
    if (i < decoded_idx ){
  ...
  contador=decoded_idx-verifica;
  if(controlo == 0 && decoded_idx == _original_message_size_ ){
    _number_of_solutions_++;
    return ;
  }
  int  ndeco=decoded_idx,nencode_idx=encoded_idx, senencode_idx=encoded_idx;
  int teste=0;
  int bitLidos=1,k=0; 
  char bits[_c_->max_bits + 1];
  while (bitLidos <= _c_->max_bits && _encoded_message_[nencode_idx] != 0 ){
    bits[k]=_encoded_message_[nencode_idx]; 
    bits[k+1]='\0';
    for(int p = 0;p < _c_->n_symbols;p++){
      teste=strcmp(bits,_c_->data[p].codeword);
        if (teste== 0) {
            _decoded_message_[ndeco]=p;     
            _number_of_calls_++;
            recursive_decoder(senencode_idx+bitLidos,ndeco+1, good_decoded_size);      
        }
    }
    bitLidos++;
    k++;
  ...
  if (contador > _max_extra_symbols_ ){
    _max_extra_symbols_=contador;
  }
   return ;
}
...
void try_it(code_t *c,int message_size,int show_results)
{
  ...
  _Finalencoded_message_ = (int *)malloc((size_t)_max_decoded_message_size_ * sizeof(int));
  ...
  free(_Finalencoded_message_);  _Finalencoded_message_  = NULL;
}
